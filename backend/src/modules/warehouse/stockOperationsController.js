...
export const getPickList = async (req, res) => {
  try {
    ...
    const serializedFlags = await resolveSerializedItemFlags({ items: order.items });
    ...
    for (const item of Array.isArray(order.items) ? order.items : []) {
      ...
      if (!sku) {
        pickList.push({
          ...
          assignedDevicesCount: Array.isArray(item.deviceAssignments) ? item.deviceAssignments.length : 0,
          locations: [],
          fulfilled: false,
      ...
      if (activeStoreId) inventoryFilter.storeId = activeStoreId;
      ...
      if (scopedRows.length === 0 && looseSku && looseSku !== strictSku) {
        const fallbackFilter = { sku: { $regex: `^0*${escapeRegex(looseSku)}$` } };
        if (activeStoreId) fallbackFilter.storeId = activeStoreId;
      ...
      if (activeStoreId) scopedRowsFilter.storeId = activeStoreId;
      ...
      const hasGoodAndPositiveWithLocationRows = allRowsScopedStore.some(
        (row) => row?.status === "GOOD" && (Number(row?.quantity) || 0) > 0 && String(row?.locationCode || "").trim()
      );
      ...
      const reason =
        remainingQty <= 0
          ...
          : resolveUnpickableReason({
              ...
              hasPositiveQuantityRows,
              hasGoodAndPositiveRows,
              hasGoodAndPositiveWithLocationRows: hasGoodAndPositiveWithLocationRows && missingLocationCount === 0,
              hasOtherStoreRows,
            });
    ...
    res.status(500).json({ success: false, message: "Lỗi khi lấy danh sách pick", error: error.message });
  }
};
...
export const pickItem = async (req, res) => {
  ...
  try {
    ...
    const resolveOrderItemSku = createOrderItemSkuResolver({ session, activeStoreId });
    let orderItem = null;
    for (const item of Array.isArray(order.items) ? order.items : []) {
    ...
    if (!orderItem) {
      await session.abortTransaction();
      return res.status(404).json({ success: false, message: `Không tìm thấy SKU ${normalizedSku} trong đơn hàng` });
    }
    ...
    const isInStoreOrder = order.orderSource === "IN_STORE" || order.fulfillmentType === "IN_STORE";
    if (isInStoreOrder) {
      ...
      if (!requestHasPermission(req, AUTHZ_ACTIONS.ORDER_PICK_COMPLETE_INSTORE, "branch")) {
        await session.abortTransaction();
        return res.status(403).json({ success: false, message: "Đơn IN_STORE chỉ cho Warehouse Manager thao tác xuất kho" });
      }

      if (assignedPickerId && actorId && assignedPickerId !== actorId) {
        await session.abortTransaction();
        return res.status(403).json({ success: false, message: "Đơn này đã được gán cho Warehouse Manager khác" });
      }
    }
    ...
    if (String(inventory.productId || "") !== String(orderItem.productId || "")) {
      await session.abortTransaction();
      return res.status(409).json({ success: false, code: "PRODUCT_MISMATCH", message: "PRODUCT_MISMATCH" });
    }
    ...
    const serializedFlags = await resolveSerializedItemFlags({ items: [orderItem], session });
    const serializedTrackingEnabled = serializedFlags.get(String(orderItem.productId || ""))?.isSerialized || false;
    ...
    if (serializedTrackingEnabled) {
      if (Array.isArray(deviceIds) && deviceIds.length > 0 && deviceIds.length !== pickQty) {
        ...
        return res.status(400).json({
          success: false,
          message: `Số lượng deviceIds phải bằng ${pickQty} cho SKU serialized`,
        });
      }
    ...
    const pickCompleted = await isOrderPickCompleted(order.items, pickedItems, { resolveOrderItemSku });
    const now = new Date();
    ...
    res.json({
      ...
      assignedDevicesCount: Array.isArray(orderItem.deviceAssignments) ? orderItem.deviceAssignments.length : 0,
    });
  } catch (error) {
...
export const transferStock = async (req, res) => {
  ...
  try {
    ...
    if (!sku?.trim() || !fromLocationCode?.trim() || !toLocationCode?.trim() || !transferQty) {
      ...
      return res.status(400).json({
        success: false,
        message: "Dữ liệu chuyển kho không hợp lệ (sku, fromLocationCode, toLocationCode, quantity)",
      });
    }

    const fromLocation = await WarehouseLocation.findOne({ storeId: activeStoreId, locationCode: fromLocationCode }).session(session);
    const toLocation = await WarehouseLocation.findOne({ storeId: activeStoreId, locationCode: toLocationCode }).session(session);
    if (!fromLocation || !toLocation) {
      await session.abortTransaction();
      return res.status(404).json({ success: false, message: "Không tìm thấy vị trí kho" });
    }

    const fromInventory = await Inventory.findOne({ storeId: activeStoreId, sku, locationId: fromLocation._id })
      .select("storeId sku quantity productId productName locationId locationCode status")
      .session(session);
    if (!fromInventory?.storeId) throw new Error("DATA_CORRUPTION: Inventory missing storeId");
    ...
    if (!fromInventory || sourceAvailableQty < transferQty) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: `Không đủ hàng tại ${fromLocationCode}. Tồn: ${sourceAvailableQty}` });
    }
    ...
    let toInventory = await Inventory.findOne({ storeId: activeStoreId, sku, locationId: toLocation._id })
      .select("storeId sku quantity productId productName locationId locationCode status")
      .session(session);
    if (toInventory) {
    ...
    await session.commitTransaction();
    res.json({ success: true, message: `Đã chuyển ${transferQty} từ ${fromLocationCode} đến ${toLocationCode}` });
  } catch (error) {
    await session.abortTransaction();
...
export const createCycleCount = async (req, res) => {
  try {
    ...
    const countNumber = `CC-${activeStore.code}-${new Date().getFullYear()}-${String(count + 1).padStart(4, "0")}`;
    ...
    for (const loc of locations) {
      const inventoryItems = await Inventory.find({ storeId: activeStoreId, locationId: loc._id })
        ...
        .lean();
      for (const inv of inventoryItems) {
        if (!inv?.storeId) throw new Error("DATA_CORRUPTION: Inventory missing storeId");
    ...
    const cycleCount = new CycleCount({
      ...
      scope:
        typeof scope === "object" && scope
          ? { warehouse: scope.warehouse || activeStore.code, zone: scope.zone || null, aisle: scope.aisle || null }
          : { warehouse: activeStore.code, zone: zones?.[0] || null, aisle: aisles?.[0] || null },
      countDate: new Date(),
      assignedTo: [{ userId: req.user._id, userName: getActorName(req.user) }],
      items,
      status: "IN_PROGRESS",
    ...
    res.status(201).json({ success: true, message: `Đã tạo phiếu kiểm kê ${countNumber} với ${items.length} mục`, cycleCount });
  } catch (error) {
...
export const completeCycleCount = async (req, res) => {
  try {
    ...
    const varianceLocations = cycleCount.items.filter((item) => item.variance !== 0 && item.countedQuantity !== null).length;
    const totalVariance = cycleCount.items.reduce((sum, item) => sum + (Number(item.variance) || 0), 0);

    cycleCount.summary = { totalLocations, matchedLocations, varianceLocations, totalVariance };
